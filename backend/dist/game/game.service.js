...
Object.defineProperty(exports, "GameService", {
    enumerable: true,
    get: function() {
        return GameService;
    }
...
let GameService = class GameService {
    ...
    async saveMatchResult(gameState) {
        if (gameState.gameMode === 'remoteMultiplayer') {
            const winner = gameState.player1.score > gameState.player2.score ? gameState.player1.id : gameState.player2.id;
    ...
    createLocalMultiplayerGame(playerId) {
        const gameId = (0, _uuid.v4)();
    ...
    createRemoteMultiplayerGame(player1Id, player2Id) {
        const gameId = (0, _uuid.v4)();
    ...
    movePaddle(playerId, gameId, direction, player) {
    ...
    startGameLoop(gameId) {
        const intervalId = setInterval(()=>{
            ...
            if (game.player1.score >= GAME_PARAMETERS.score_limit || game.player2.score >= GAME_PARAMETERS.score_limit) {
                const winner = game.player1.score >= GAME_PARAMETERS.score_limit ? 'player1' : 'player2';
    ...
    updateGame(gameId, game) {
        ...
        if (game.powerUp) {
            ...
            if (ballRight > powerUpLeft && ballLeft < powerUpRight && ballBottom > powerUpTop && ballTop < powerUpBottom) {
                const affectedPlayer = game.ball.velocityX > 0 ? game.player1 : game.player2;
    ...
    checkPaddleCollision(paddle, ball) {
        ...
        if (ballRight > paddleLeft && ballTop < paddleBottom && ballLeft < paddleRight && ballBottom > paddleTop) {
    ...
    requestRematch(gameId, playerId) {
        const game = this.games.get(gameId);
        if (!game) {
            return {
        ...
        if ((gameMode === 'singleplayer' || gameMode === 'localMultiplayer') && requests.length >= 1) {
    ...
    resetBall(ball, game) {
        ...
        ball.velocityX = GAME_PARAMETERS.ball.initialVelocityX * (ball.velocityX > 0 ? -1 : 1);
        ball.velocityY = GAME_PARAMETERS.ball.initialVelocityY * (ball.velocityY > 0 ? -1 : 1);
        ball.speed = GAME_PARAMETERS.ball.initialSpeed;
        game.powerUp = undefined;
        game.lastPowerUpSpawn = undefined;
        return Date.now();
    }
    getPlayerIndex(playerId, gameId) {
        const game = this.games.get(gameId);
        if (!game) return -1;
        let index = 0;
    ...
    constructor(matchModel){
        ...
        this.logger = new _common.Logger(GameService.name);
    }
};
GameService = _ts_decorate([
    (0, _common.Injectable)(),
    _ts_param(0, (0, _sequelize.InjectModel)(_matchentity.Match)),
