...
const UserPage: React.FC = () => {
  ...
  const { isConnected, socketId } = useSocketStatus(user ? user.username : undefined);
  ...
  const handlegototchats = () => {
    if (user) navigate("/chat", { state: { user: user } });
  };
  const handlegototgame = () => {
    if (user) navigate("/game", { state: { user: user, requestedUser: null } });
  };

  const handleGoToLeaderboard = () => {
    if (user) {
      navigate("/leaderboard", { state: { user: user } });
    }
  };
    ...
    <div style={{ padding: "20px" }}>
      ...
      {user ? (
        <div>
          ...
          <p>
            <strong>Image Name:</strong> {user.imageName}
          </p>
      ...
      <button onClick={handlegototchats}>Chat</button>
      <button onClick={handlegototgame}>Game</button>
